...
> as a #348 companion — the §5.5 "what do we already own?" check, applied at
...
| ~~2~~ | ~~**`defer catch` / `defer try`** (§6)~~ | ✅ **SHIPPED** | PR #1140, v0.396.0. The groundwork found and fixed a live leak first: `expr!` propagation drained no defers at all (PR #1137) |
| 3 | **`@nodiscard`** (§7) | Tiny | Free leak guard |
| ~~4~~ | ~~**Compile-time `where`/contract folding** (§10.2)~~ | ✅ **SHIPPED** | Designed first (docs/contract-folding.md) — which found HALF of it already existed (constant-true elision, #348 companion). Both tiers shipped, error-from-day-one |
