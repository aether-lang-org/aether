...
**Known limitation — ownership through a `string` parameter.** A value transferred into a container *through a `string` parameter* of a storing wrapper (`pr(m, k, v) { map.put(m, k, v) }`) is the unproven case: the put site sees only a `string` param, and a parameter can hold either an owned heap string the caller minted *or* a borrowed/literal one. The codegen leaves it on the non-owning path, so when the caller did mint and transfer an owned value it leaks (the caller is escape-marked, the container doesn't free). Dispatching at runtime on the AetherString magic header is **not** a sound fix: the magic header proves heap *representation*, not caller-transferred *ownership*, so a borrowed magic string (still owned by another container/scope) would be double-freed. Representation is not ownership — and a double-free is worse than a leak. The sound fix is an explicit ownership-transfer parameter annotation (e.g. `@owned` / `@consume`) that routes only opt-in transferred values to the owning put; until then these few cases are tracked in `tests/leaks_known.txt`.
