...
# Regenerated whenever $(VERSION) changes; written atomically (tmp + mv) and
# only rewritten on content change, so it never spuriously dirties the tree or
# triggers rebuilds. GENERATED — do not commit / edit by hand (gitignored).
...
compiler: gen-version-header $(COMPILER_OBJS) $(STD_OBJS) $(COLLECTIONS_OBJS) $(OBJ_DIR)/runtime/aether_sandbox.o $(OBJ_DIR)/runtime/aether_resource_caps.o $(IO_POLLER_OBJS)
...
stdlib: gen-version-header $(STD_OBJS) $(STD_REACTOR_OBJS) $(COLLECTIONS_OBJS) $(RUNTIME_OBJS) build/libaether_compiler.a build/MANIFEST
...
# Install to system
PREFIX ?= /usr/local
install: gen-version-header release ae stdlib
	@echo "==================================="
